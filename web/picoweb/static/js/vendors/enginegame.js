function engineGame(options) {
    options = options || {};
    var engine_game = new Chess();
    var board = options.board;
    var engine = new Worker(options.stockfishjs || '/static/js/stockfish.js');
    var engineStatus = {};
    ...
    // do not pick up pieces if the engine_game is over
    // only pick up pieces for White
    var onDragStart = function(source, piece, position, orientation) {
        var re = playerColor == 'white' ? /^b/ : /^w/
            if (engine_game.game_over() ||
                piece.search(re) !== -1) {
                return false;
            }
    };
    ...
    function prepareMove() {
        stopClock();
        $('#pgn').text(engine_game.pgn());
        board.position(engine_game.fen());
        updateClock();
        var turn = engine_game.turn() == 'w' ? 'white' : 'black';
        if(!engine_game.game_over()) {
            if(turn != playerColor) {
                var moves = '';
                var history = engine_game.history({verbose: true, raw: false});
            ...
            if(engine_game.history().length >= 2 && !time.depth && !time.nodes) {
                startClock();
            }
        }
    }
    ...
    engine.onmessage = function(event) {
        ...
        if(line == 'uciok') {
            ...
            if(match) {
                ...
                engine_game.move({from: match[1], to: match[2], promotion: match[3]});
                prepareMove();
            } else if(match = line.match(/^info .*\bdepth (\d+) .*\bnps (\d+)/)) {
                engineStatus.search = 'Depth: ' + match[1] + ' Nps: ' + match[2];
            }
            if(match = line.match(/^info .*\bscore (\w+) (-?\d+)/)) {
                var score = parseInt(match[2]) * (engine_game.turn() == 'w' ? 1 : -1);
                if(match[1] == 'cp') {
                ...
                if(match = line.match(/\b(upper|lower)bound\b/)) {
                    engineStatus.score = ((match[1] == 'upper') == (engine_game.turn() == 'w') ? '<= ' : '>= ') + engineStatus.score
                }
            }
        }
        displayStatus();
    ...
    return {
        ...
        loadPgn: function(pgn) { engine_game.load_pgn(pgn); },
        ...
        setTime: function(baseTime, inc, bonus) {
            time = { wtime: baseTime * 1000, btime: baseTime * 1000, winc: inc * 1000, binc: inc * 1000 };
            if (bonus ) {
                if (bonus.white) {
                    time.wtime += +bonus.white * 1000;
                }
                else if (bonus.black) {
                    time.btime += +bonus.black * 1000;
                }
            }
        },
