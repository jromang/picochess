...
class Message():
    ...
    INTERACTION_MODE = ClassFactory(MessageApi.INTERACTION_MODE, ['mode', 'mode_string', 'beep'])
...
class Event():
    ...
    SET_OPENING_BOOK = ClassFactory(EventApi.SET_OPENING_BOOK, ['book', 'book_string', 'beep'])
    NEW_ENGINE = ClassFactory(EventApi.NEW_ENGINE, ['eng', 'level'])
    SET_INTERACTION_MODE = ClassFactory(EventApi.SET_INTERACTION_MODE, ['mode', 'mode_string', 'beep'])
    ...
    SET_TIME_CONTROL = ClassFactory(EventApi.SET_TIME_CONTROL, ['time_control', 'time_string', 'beep'])
...
def which(program):
    ...
    if fpath:
        ...
        for path in os.environ["PATH"].split(os.pathsep) + [pe, pe + os.sep + 'engines' + os.sep + platform.machine()]:
            path = path.strip('"')
