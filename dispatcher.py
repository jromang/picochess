...
class Dispatcher(DispatchDgt, Thread):
    ...
    def _process_message(self, message, dev: str):
        do_handle = True
        if repr(message) in (DgtApi.CLOCK_SET, DgtApi.CLOCK_START, DgtApi.CLOCK_STOP):
        ...
        if do_handle:
            ...
            clk = (DgtApi.DISPLAY_MOVE, DgtApi.DISPLAY_TEXT, DgtApi.DISPLAY_TIME, DgtApi.CLOCK_SET, DgtApi.CLOCK_START, DgtApi.CLOCK_STOP)
            ...
            if repr(message) in (DgtApi.CLOCK_SET, DgtApi.CLOCK_START) and self.dgtmenu.inside_updt_menu():
                logging.debug('(%s) inside update menu => clock not set/started', dev)
                return
